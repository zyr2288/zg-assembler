...
在编辑器（默认是左边，与文件列表一起）有标签树功能，点击标签会跳转到标签所在位置，编译过后能够导出标签对应的地址。
...
可以使用插件，具体使用方法可以查看[内置插件](./doc/内置插件.md)
...
此时，"中"的对应值为 `0`，"文"的对应值为 `1`, "{结束}" 的对应值为 `2`，即结果为 `00 01 02`

当字符码表中没有对应的字符时，则默认输出当前字符的 Unicode 或者 0（如果是控制符，例如 {结束}）
...
此时已经锁定了字符固定长度，所以结果为 `00 00 00 01 00 02 00 03`。
...
- 当为 `@` 开头，则为项目的根目录下的文件路径。

例如:

```
    .INCBIN "文件夹/文件.bin", 0, 100
	.INCBIN "@/文件夹/文件.bin", 0, 200
```
...
- 编译之后结果为：`12 34 56 78 90 AB CD EF`
